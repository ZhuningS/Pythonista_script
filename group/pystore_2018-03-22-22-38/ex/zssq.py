...
SYSTEM_STYLE_DATA = { '追书红':'#992300', '青草绿':'#1c7b37', '少女粉':'#b43e82','水鸭青':'#008080', '摩卡棕':'#605300' }
READER_STYLE_DATA = { '羊皮棕':'#a28a6a', '豆沙绿': '#c7edcc','杏仁黄': '#ccc8ad', '玫瑰棕': '#a27b7b', '白银灰': '#C0C0C0' }
...
def search_books (keywords):
    ...
    url = 'http://api.zhuishushenqi.com/book/fuzzy-search?query={}&start=0&limit=100'.format(keywords)
    res = api(url) or exit()
    books = json.loads(res)['books']
    booksearch_data = { book['_id']:book
    for book in books }
    booksearch_list = [ book['_id'] for book in books ]

def get_book_list (mode,num):
    urls = {
            'week': 'https://api.zhuishushenqi.com/book-list?sort=collectorCount&duration=last-seven-days&start=',
            'new': 'https://api.zhuishushenqi.com/book-list?sort=created&duration=all&start=',
            'top': 'https://api.zhuishushenqi.com/book-list?sort=collectorCount&duration=all&start=' }
...
def get_booklist_books (booklist_id):
    ...
    booklist_books_list = [ item['book']['_id'] for item in booklist_data['books'] ]
    booklist_books_data = { item['book']['_id']: item['book'] for item in booklist_data['books'] }
...
def get_top_book ():
    ...
    booktop_name = [ top['title']
    for top in json.loads(res1)['male']
            if top['_id']==top_id ][0]
...
def get_book_sources (book_id):
    ...
    url = 'http://api.zhuishushenqi.com/toc?view=summary&book={}'.format(book_id)
    res = api(url) or exit()
...
def get_book_chapters (source_id):
    ...
    chapterslist = [ '{}#{}'.format(idx,val['title']) for idx,val in enumerate(chapters) ]
...
def get_chapter_content (chapter_link):
    ...
    url = 'http://chapterup01.zhuishushenqi.com/chapter/{}?t={}'.format(chapter_url,int(time.time()+7200))
    res = api(url) or exit()
    try:
...
def string_split (string):
    str_list = [ '&#12288;&#12288;{}<br/>'.format(str) for str in string.split('\n') ]
...
def bookcase_refresh ():
    ...
    for id in bookcase_list:
        ...
        befor_time = time.mktime(time.strptime(befor_time,'%Y-%m-%d %H:%M:%S'))
        after_time = time.mktime(time.strptime(after_time,'%Y-%m-%d %H:%M:%S'))
        now_time = time.time()
...
class KeyWordsDelegate (object):
    ...
    def textfield_did_change(self,textfield):
        if not textfield.text:
            searchview.done_button.image = ui.Image.named('iow:ios7_refresh_32')
            ...
            searchview.done_button.image = ui.Image.named('iob:ios7_search_strong_32')
...
class ReaderView (ui.View):
    ...
    def draw (self):
        ...
        if self.mode == 'source':
            ...
            self.chapter_title = '{}#{}'.format(chapters_count,chapters_data[chapters_count]['title'])
            self.chapter_num = int(self.chapter_title.split('#')[0])
            self.mode = None
        else:
            self.chapter_title = '{}#{}'.format(self.chapter_num,chapters_data[self.chapter_num]['title'])
        ...
        if chapterview.mode == 'local' and 'page_log' in bookcase_data[chapterview.book_id] and bookcase_data[chapterview.book_id]['chapter_log'] == self.chapter_title:
            self.page = bookcase_data[chapterview.book_id]['page_log']
        else:
            self.page = 0
        self.html = self.TEMPLATE.replace('{{CONTENT}}',self.txt).replace('{{STYLE}}',READER_STYLE).replace('{{chapter}}',self.chapter_title.split('#')[1]).replace('{{page}}',str(self.page))
    ...
    def touch_ended (self, touch):
        ...
        if w/3*2 > x > w/3 and y > h/3*2:
            ...
            if 'touch_time' in globals() and len(touch_time) == 2 and touch_time[1]-touch_time[0] <= 0.2:
                self.close()
                del touch_time[0]
            elif 'touch_time' in globals() and len(touch_time) == 2 and touch_time[1]-touch_time[0] > 0.2:
                del touch_time[0]
...
class ChapterView (ui.View):
    ...
    def __init__ (self, book_id, mode):
        ...
        try:
            ...
            self.source_id = get_book_sources(self.book_id)[self.source_name]['_id']
        except:
...
class SearchDataSource (object):
    ...
    def tableview_title_for_header(self, tableview, section):
        if self.mode == 'search':
            ...
            title = booktop_name if 'booktop_name' in globals() else '正在加载Top数据...'
            return title
        else:
            return False
...
class BookDetailView (ui.View):
    ...
    def __init__ (self, book_id, mode):
        ...
        self.detail.text = '作者：{}\n\n分类：{}\n\n总字数：{}\n\n最新章节：{}'.format(self.book_info.get('author','无信息'),self.book_info.get('cat','无信息'),self.book_info.get('wordCount','无信息'),self.book_info.get('lastChapter','无信息'))
        ...
        self.follower.text = '{:.1f}万人气'.format(self.book_info['latelyFollower']/1000)
...
class BookListView (ui.View):
    ...
    def button_tapped (self, sender):
        if sender.name == 'top':
            ...
            self.list.content_offset = (0,self.list.content_size[1]-self.list.height)
...
class BookListDelegate (object):
    ...
    def scrollview_did_scroll (self, scroll):
        if scroll.content_offset[1] > (len(book_lists_list)-20)*100+1483+60:
...
class BookListBooksView (ui.View):
    def __init__ (self):
        ...
        self.table = ui.TableView()
        self.table.frame = (0,self.info.y+self.info.height+0,w,h-self.info.y-self.info.height-125)
        ...
        oval = Oval(self.bg_color,self.avatar.x,self.avatar.y,self.avatar.width,self.avatar.height)
    ...
    def top_display (self):
        self.table.frame = (0,self.info.y+self.info.height+10,w,h-self.info.y-self.info.height-125)
...
root_view.present('popover',hide_title_bar=True)
